...
"""
    Esta clase se encarga de hacer un Edge Slipt a la malla del modelo, consite en subdividir 
    una cara en 4. 
    Se utiliza OpenGL, para rendeizar en tiempo real el modelo, tambien posee una luz ambiental, 
    la camara se posiciona a una distancia siempre alejada del objeto, a pesar de us tamaño. 
"""


class RendererEdgeSplit:
    def __init__(self):
        self.initialize_edge_split_scene()
    ...
    def initialize_edge_split_scene(self):
        pygame.init()
        display = (600, 500)
        self.normal_window = pygame.display.set_mode(display, DOUBLEBUF | OPENGL)
        pygame.display.set_caption("Visualizador 3D - Edge Split")
    ...
    def apply_edge_split(self, vertices, triangles):
        new_vertices = vertices[:]  # Copia de la lista de vértices
        new_triangles = []
        for tri in triangles:
            ...
            # Crear cuatro nuevos triángulos a partir del original
    ...
    def render_with_edge_split(self, vertices, faces, edges, tam):
        while True:
            self.handle_events()
