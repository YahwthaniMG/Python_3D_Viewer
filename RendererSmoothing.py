...
"""
    Esta clase se encarga de renderizar el modelo despues de aplicar el suavizado laplaciano.
...
class RendererSmoothing:
    def __init__(self):
        self.initialize_smoothed_scene()
    ...
    def apply_laplacian_smoothing(self, vertices, faces, iterations=1, factor=0.5):
        smoothed_vertices = vertices.copy()  # Copia de los vértices originales
        # Convertir vértices y caras a numpy arrays para un manejo más eficiente
        vertices_np = np.array(vertices)
        faces_np = np.array(faces)
        # Crear una matriz de adyacencia para almacenar los índices de los vértices vecinos
        adjacency_matrix = [[] for _ in range(len(vertices_np))]
        ...
        # Aplicar el suavizado Laplaciano
    ...
    def initialize_smoothed_scene(self):
        pygame.init()
        display = (600, 500)
        self.normal_window = pygame.display.set_mode(display, DOUBLEBUF | OPENGL)
        pygame.display.set_caption("Visualizador 3D - Objeto Suavizado")
    ...
    def render_with_smoothing(self, vertices, faces, edges, tam):
